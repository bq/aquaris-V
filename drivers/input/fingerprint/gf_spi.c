...
{
	seq_printf(file,"[Vendor]gf3258,Goodix\n");
	return 0;
}
...
// add for fingerprint check list end
	...
	case GF_IOC_INPUT_KEY_EVENT:
		...
		for(i = 0; i< ARRAY_SIZE(maps); i++) {
          //  printk("maps[%d].code] = %u  gf_key.key = %u  GF_KEY_CAMERA = %u\n",i,maps[i].code,gf_key.key,GF_KEY_CAMERA);
		//	if(maps[i].code == GF_KEY_CAMERA){
          //  printk("gf3208 gf_key->key = %u\n",gf_key.key);
            if (GF_KEY_HOME == gf_key.key) {
